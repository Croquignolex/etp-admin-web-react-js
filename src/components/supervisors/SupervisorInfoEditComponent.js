...
// Component
function SupervisorInfoEditComponent({request, supervisor, dispatch, handleClose}) {
    // Local state
    const [name, setName] = useState({...DEFAULT_FORM_DATA, data: supervisor.name});
    const [email, setEmail] = useState({...DEFAULT_FORM_DATA, data: supervisor.email});
    const [address, setAddress] = useState({...DEFAULT_FORM_DATA, data: supervisor.address});
    const [description, setDescription] = useState({...DEFAULT_FORM_DATA, data: supervisor.description});
    ...
    const shouldResetErrorData = () => {
        dispatch(storeSupervisorEditInfoRequestReset());
    };
    ...
    const handleSubmit = (e) => {
        ...
        // Check
        if(validationOK) {
            dispatch(emitUpdateSupervisorInfo({
                name: _name.data,
                id: supervisor.id,
                email: email.data,
...
SupervisorInfoEditComponent.propTypes = {
...
export default React.memo(SupervisorInfoEditComponent);
