...
function reduce(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case actions.STORE_MANAGER_EDIT_INFO_REQUEST_RESET:
            ...
            return nextState || state;
        // ======================================================== Reset manager
        // Resolve event to set reset manager init request store data
        case actions.STORE_RESET_MANAGER_REQUEST_INIT:
            nextState = {...state, reset: requestInitValue()};
            return nextState || state;
        // Resolve event to set reset manager failed request store data
        case actions.STORE_RESET_MANAGER_REQUEST_FAILED:
            nextState = {...state, reset: requestFailedValue(action.message)};
            return nextState || state;
        // Resolve event to set reset manager succeeded request store data
        case actions.STORE_RESET_MANAGER_REQUEST_SUCCEEDED:
            nextState = {...state, reset: requestSucceededValue(action.message)};
            return nextState || state;
        // Resolve event to set reset manager reset request store data
        case actions.STORE_RESET_MANAGER_REQUEST_RESET:
